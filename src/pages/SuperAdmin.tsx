...
import { Plus, Edit, Trash2, AlertCircle, User as UserIcon, LayoutGrid, Users, Loader2, Copy, UserPlus } from 'lucide-react';
...
const SuperAdmin = () => {
  ...
  const renderAllUsersList = () => {
    ...
    if (!allRegularUsers || allRegularUsers.length === 0) { return <TableRow><TableCell colSpan={6} className="text-center h-24">No regular users found in the system.</TableCell></TableRow>; }
    return allRegularUsers.map((user) => (
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <Tabs defaultValue="all-users" className="w-full">
                ...
                <TabsContent value="all-users" className="space-y-6 mt-6">
                  <Card>
                    <CardHeader><div className="flex items-center justify-between"><div><CardTitle className="flex items-center gap-2"><Users /> All Users</CardTitle><CardDescription>A complete list of all regular (non-admin) users in the system.</CardDescription></div><Button onClick={() => setIsAddUserDialogOpen(true)}><UserPlus className="mr-2 h-4 w-4"/>Add User</Button></div></CardHeader>
      ...
      <Dialog open={isAdminFormOpen} onOpenChange={setIsAdminFormOpen}>
        <DialogContent className="sm:max-w-[425px]"><DialogHeader><DialogTitle>{editingAdmin ? 'Edit Administrator' : 'Add New Administrator'}</DialogTitle></DialogHeader>
            <form onSubmit={adminForm.handleSubmit(data => upsertAdminMutation(data))} className="space-y-4 pt-4">
                ...
                <div><Label>Assign Categories</Label><div className="space-y-2 rounded-md border p-4 max-h-40 overflow-y-auto"><Controller name="categories" control={adminForm.control} render={({ field }) => (<>{isLoadingCategories ? <Skeleton className='h-5 w-20'/> : categories?.map((category) => (<div key={category._id} className="flex items-center space-x-2"><Checkbox id={category._id} checked={field.value?.includes(category.name)} onCheckedChange={(checked) => { const current = field.value || []; const newCategories = checked ? [...current, category.name] : current.filter(name => name !== category.name); field.onChange(newCategories);}}/><label htmlFor={category._id} className="text-sm font-medium">{category.name}</label></div>))}</>)}/></div></div>
                <DialogFooter><Button type="button" variant="secondary" onClick={() => setIsAdminFormOpen(false)}>Cancel</Button><Button type="submit" disabled={isUpsertingAdmin}>{isUpsertingAdmin ? "Saving..." : "Save Changes"}</Button></DialogFooter>
            </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isCategoryFormOpen} onOpenChange={setIsCategoryFormOpen}>
        <DialogContent><DialogHeader><DialogTitle>Add New Category</DialogTitle></DialogHeader>
            <form onSubmit={categoryForm.handleSubmit(data => addCategoryMutation(data))} className="space-y-4 pt-4">
                ...
                <DialogFooter><Button type="button" variant="secondary" onClick={() => setIsCategoryFormOpen(false)}>Cancel</Button><Button type="submit" disabled={isAddingCategory}>{isAddingCategory ? "Adding..." : "Add Category"}</Button></DialogFooter>
            </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isAddUserDialogOpen} onOpenChange={setIsAddUserDialogOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={addUserForm.handleSubmit(data => addUserMutation(data))} className="space-y-4 pt-4">
              ...
              <DialogFooter><Button type="button" variant="secondary" onClick={() => setIsAddUserDialogOpen(false)}>Cancel</Button><Button type="submit" disabled={isAddingUser}>{isAddingUser ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Creating...</> : "Create User"}</Button></DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!createdUserInfo} onOpenChange={() => setCreatedUserInfo(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className='flex items-center gap-2'><UserPlus className='w-5 h-5 text-green-600'/>User Created Successfully</DialogTitle><DialogDescription>Please share these temporary credentials with the user.</DialogDescription></DialogHeader>
          <div className="space-y-4 py-4">
            <p><strong>Name:</strong> {createdUserInfo?.name}</p>
            <p><strong>Email:</strong> {createdUserInfo?.email}</p>
            <div className='flex items-center gap-2'>
              <p><strong>Password:</strong> <span className="font-mono bg-gray-100 p-1 rounded">{createdUserInfo?.password_was}</span></p>
          ...
          <DialogFooter><Button onClick={() => setCreatedUserInfo(null)}>Close</Button></DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={editSubscriptionsForm.handleSubmit(onSubscriptionSubmit)} className="space-y-4 pt-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={isUpdatingSubscriptions}>{isUpdatingSubscriptions ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Saving...</> : "Save Changes"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
